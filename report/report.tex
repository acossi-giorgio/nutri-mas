...
responses, before publishing the result in the UI.
...
mapped with static message through a \texttt{CyclicBehaviour}.
...
The content of the template therefore influences
...
The BDI plans establish the decisions, while the Python actions calculate the targets per
...
\begin{itemize}
    ...
    \item \textbf{Nutritional tolerance}: \texttt{template\_candidate} combines calories and macronutrients of the template with the received target. The beliefs \texttt{tollerance} delimit the acceptable intervals before a candidate can be selected.
    \item \textbf{Guided category}: when the Planner still has to satisfy a minimum frequency, \texttt{guided\_template\_candidate} adds the required category to the selection constraints.
    ...
    \item \textbf{Explicit failure}: if no candidate satisfies all the constraints, the Chef returns an empty domain or an explicit failure instead of relaxing the nutritional criteria.
    \item \textbf{Meal alternative}: \texttt{choose\_runtime\_template} applies \texttt{template\_candidate} to select a proposal different from the planned meal while preserving diet, slot and nutritional constraints.
\end{itemize}
...
mainly for tasks that were not central to its research objectives. Their use was limited to the following activities:
...
\begin{itemize}
    ...
    \item \textbf{Debugging support}: AI tools were occasionally used to investigate complex implementation problems. 
    This was particularly useful when debugging AgentSpeak plans, where untyped terms, complex predicates and 
    interactions between multiple plans can make errors difficult to identify.
    \item \textbf{Documentation}: AI assistance was used to improve the clarity, structure and language of 
    selected parts of the project documentation.
\end{itemize}
